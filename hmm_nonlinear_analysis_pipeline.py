...
def plot_nonlinear_regression_comparison(df3, shuff, subject_col, group_cols, trial_col, value_col, flag= None, yMin=None, yMax=None, nIter=100, parallel=True, textsize=20):
    ...
    ta.plot_fits_summary_avg(df3, shuff_df=shuff, dat_col=value_col, trial_col=trial_col, save_dir=HA.save_dir,
                             use_alpha_pos=False, textsize=textsize, dotalpha=0.15, flag=flag, nIter=nIter, parallel=parallel, yMin=yMin, yMax=yMax)
    ...
    ta.plot_r2_pval_diffs_summary(avg_shuff, df3, save_flag=save_flag, save_dir=HA.save_dir, textsize=textsize, nIter=nIter)#re-run and replot with nIter=nIter


def plot_nonlinear_regression(df3, shuff, subject_col, group_cols, trial_col, value_col, flag=None, nIter=100, parallel=True, textsize=20):
    groups = [subject_col] + group_cols
    avg_shuff = shuff.groupby(group_cols + ['iternum']).mean().reset_index()
