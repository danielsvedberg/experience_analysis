...
proj_dir = '/home/senecascott/Documents/AviData/taste_experience_resorts'
...
for name, group in held_df_long.groupby(['rec_dir']):
    ...
    #add the nth taste exposure as a column to digintrials
    digintrials['tasteExposure#'] = digintrials.groupby(['name', 'channel']).cumcount()+1
    
    #delete the weird column at the top
    ...
    for i , row in group.iterrows():
        ...
        for j, dinrow in dinmap[:4].iterrows():
            ...
            #best_hmm_row = best_hmms.loc[best_hmms.rec_dir==row.rec_dir]
            #best_hmm_row = best_hmm_row.loc[best_hmm_row.channel==int(dinrow.channel)]
            #rate = hmma.get_state_firing_rates(row.rec_dir, int(best_hmm_row.hmm_id), int(best_hmm_row.late)) #gives a list with firign rates for each trial and neuron number
