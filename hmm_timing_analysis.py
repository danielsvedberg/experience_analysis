...
timing = hmma.get_NB_states_and_probs(HA)
#%%
...
NB_split_aov['significant'] = NB_split_aov['p-GG-corr'] < (0.05/3)
...
for name, group in NB_split_aov.groupby(['trial_split', 'session']):
    ...
    avg['significant'] = combined_pval[1] < (0.05/3)
    avg_df.append(avg)
...
for i, epoch in enumerate(epochs):#(epoch, group) in enumerate(full_df.groupby('epoch')):
    ...
    for j in [1,2,3]:
        ...
        for k, (sig, grp) in enumerate(group.groupby('significant')):
            ...
            ax.axhline((0.05/3), color='b', linestyle='--')
        if i == 0:
